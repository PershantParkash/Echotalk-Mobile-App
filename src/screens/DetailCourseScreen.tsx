...
import useRatingService from "../services/rating";
...
const DetailCourseScreen: React.FC = () => {
  ...
  const { getCourseRating } = useRatingService();
  ...
  const fetchCourseReviews = async () => {
    try {
      if (courseId) {
        const reviews = await getCourseRating(Number(courseId));
          ...
          <View className="px-5 pb-6">
            <View className="flex-row items-center justify-between mb-6">
              <View>
                <Text className="text-4xl font-bold text-gray-900">
                  {(() => {
                    const rating = averageRating > 0 ? averageRating : (course?.rating || 0);
                    return Number(rating).toFixed(1);
                  })()}
                </Text>
                {renderStars(Math.round(averageRating > 0 ? averageRating : (course?.rating || 0)))}
                <Text className="text-gray-500 text-sm mt-1">
                  ({courseReviews.length > 0 ? courseReviews.length : (course?.ratingCount || 0)} reviews)
                </Text>
              </View>
            </View>
            ...
            {courseReviews.length > 0 ? (
                ...
                >
                  <View className="flex-row items-center justify-between mb-2">
                    <View className="flex-row items-center">
                      <Image
                        source={{
                          uri: review.user.profileImage || `https://i.pravatar.cc/100?img=${review.id}`,
                        }}
              ...
              <View className="items-center justify-center py-8">
                ...
                <Text className="text-gray-400 text-sm mt-1">Be the first to review this course!</Text>
              </View>
            )}
          </View>
        );
